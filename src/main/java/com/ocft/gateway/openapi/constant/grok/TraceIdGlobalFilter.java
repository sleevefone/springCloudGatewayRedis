...
public class TraceIdGlobalFilter implements GlobalFilter, Ordered {
    ...
    // 创建 ContextSnapshotFactory（单例，复用）
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return Mono.defer(() -> {
                    // 使用 ContextSnapshotFactory 替换 setAllThreadLocalsFrom
                    try (ContextSnapshot.Scope scope = snapshotFactory.setThreadLocalsFrom(
                ...
                .doFinally(signalType -> {
                    // 可选：清理 MDC（ContextSnapshot 的 Scope 自动清理）
